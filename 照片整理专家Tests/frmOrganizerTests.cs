...
{
    [TestClass()]
    public class frmOrganizerTests
    {
        [TestMethod()]
        public void GetExifByMeTest()
        {

            for (int i = 3; i < 80; i++)
            {
                string str = GetRandomizer(i, true, false, true, true);
                try
                {
                    Console.WriteLine("{0} 截取后：{1}" ,str, str.Length < 50 ? str : str.Substring(0, 12) + "..." + str.Substring(str.Length - 33, 33));
                }
                catch (Exception)
                {
                    Console.WriteLine(str + "截取时报错");
                    Assert.Fail();
                }
            }
        }
